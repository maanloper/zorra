...
restore_backup(){
	...
	for backup_dataset in ${backup_datasets}; do
		...
		if [[ "${backup_dataset_origin}" == "-" ]]; then
			...
			## Execute a full send
			zfs send -v -w -p -b "${oldest_backup_snapshot}" | ${ssh_prefix} sudo zfs receive "${source_dataset}"
		...
		if [[ "${latest_backup_snapshot#*@}" != "${latest_source_snapshot#*@}" ]]; then
			zfs send -v -w -p -b -I "${latest_source_snapshot}" "${latest_backup_snapshot}" | ${ssh_prefix} sudo zfs receive ${origin_property} "${source_dataset}"
		else
