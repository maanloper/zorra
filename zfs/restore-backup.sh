...
restore_backup(){
	...
	for backup_dataset in ${backup_datasets}; do
		...
		if [[ "${backup_dataset_origin}" == "-" ]]; then
			...
			## Execute a full send
			zfs send -w -p -b "${oldest_backup_snapshot}" | ${ssh_prefix} sudo zfs receive -v "${source_dataset}" 2>&1 && exit 0 || (grep -q "nvlist_lookup_string" || (grep -v "nvlist_lookup_string" && exit 1))
