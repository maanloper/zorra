...
restore_backup(){
	...
	for backup_dataset in ${backup_datasets}; do
		...
		if [[ "${backup_dataset_origin}" == "-" ]]; then
			...
			## Execute a full send
			zfs send -w -p -b "${oldest_backup_snapshot}" | ${ssh_prefix} sudo zfs receive -v "${source_dataset}" 2> >(grep -v "fnvlist_lookup_string" || true)
		...
		if [[ "${latest_backup_snapshot#*@}" != "${latest_source_snapshot#*@}" ]]; then
			zfs send -w -p -b -I "${latest_source_snapshot}" "${latest_backup_snapshot}" | ${ssh_prefix} sudo zfs receive -v ${origin_property} "${source_dataset}" 2> >(grep -v "fnvlist_lookup_string" || true)
		else
