...
restore_backup(){
	...
	for backup_dataset in ${backup_datasets}; do
		...
		if [[ "${backup_dataset_origin}" == "-" ]]; then
			...
			## Execute a full send
			zfs send -w -p -b "${oldest_backup_snapshot}" | ${ssh_prefix} sudo zfs receive -v "${source_dataset}" | grep -v -e "nvlist_lookup_string"  | tee /dev/tty | grep -q -e "cannot" -e "error" && exit 1 || true
